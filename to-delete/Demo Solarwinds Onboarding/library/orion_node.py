...
def run_module():
    ...
    module_args = {
        ...
        'snmp_allow_64': {'type': bool,'required': False, 'default': True},
        'wmi_credential': {'required': False, 'no_log': True},
        'polling_engine': {'required': False},
        'custom_properties': {'type': dict, 'required': False, 'default': {}}
...
def _validate_fields(module):
    ...
    if not props['ObjectSubType']:
        module.fail_json(msg='Polling Method is required [SNMP, ICMP, WMI, Agent]')
...
def _add_pollers(module, nodeid):
...
def add_node(module):
    ...
    # Add Node
    try:
        response = __SWIS__.create('Orion.Nodes', **props)
        sresp = response.split("=")
        nodeid = sresp[-1]
        poll_response = __SWIS__.invoke('Orion.Nodes', 'PollNow', 'N:' + nodeid)
    except Exception as e:
